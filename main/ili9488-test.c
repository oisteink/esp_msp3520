...
void app_main(void)
{
    ...
    uint16_t z = touch_z_threshold_load(CONFIG_XPT2046_Z_THRESHOLD);
    esp_lcd_touch_xpt2046_set_z_threshold(z);
    ESP_LOGI(TAG, "Touch z_threshold=%u", z);

    // Console
    ESP_LOGI(TAG, "starting console");
    console_set_cal_context(&app_ctx, &lvgl_lock, LCD_H_RES, LCD_V_RES);
    console_init(&app_ctx);
}
